...
def main():
	...
	list_path = '../../davsn/dataset/SynthiaSeq_list/train.txt'
	...
	with torch.no_grad():
		for ind, file_name in enumerate(file_names):
			...
			frame_prev = frame-gap
			if frame_prev <= 0:
				frame_prev = 0
			file_name1 = file_name.replace(str(frame).zfill(6) + '.png', str(frame_prev).zfill(6) + '.png')
			...
			pred_img1_name = file_name.split('/')[-1].replace('.png', '_' + str(frame_prev).zfill(6) + '.jpg')
			...
			flow_color_img1_name = file_name.split('/')[-1].replace('.png', '_'+str(frame_prev).zfill(6)+'_flow_color.jpg') ##!! add previous frame in flow as well
			cv2.imwrite(os.path.join(save_dir, flow_color_img1_name), flow_color_img1)

			flow_img1_int16_x10 = np.int16(flow_img1_np*10)
			flow_img1_int16_name = file_name.split('/')[-1].replace('.png', '_'+str(frame_prev).zfill(6)+'_int16_x10')
